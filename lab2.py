...
import types
          ...
          # BULLY ALGORITHM USING THREADING
...
class ElectManager:
    ...
    async def elect_and_announce(self):
        ...
        if (is_leader):
            ...
            # open thread async and no need to wait.
            ...
            for member in self.members:
                if compare_id(member, self.process_id) != 0:
                    task = self.loop.run_in_executor(
                        self.executor, self.send_announce_async, member)
                    tasks.append(task)
    ...
    async def elect(self):
        ...
        for member in self.members:
            if compare_id(member, self.process_id) > 0:
                task = self.loop.run_in_executor(
                    self.executor, self.send_elect_async, member)
                tasks.append(task)
        results = []
        if (len(tasks) > 0):
    ...
    def connect_and_send(self, peer, data):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as peer_sock:
            ...
            try:
                ...
                log_warn("Client Warning : Failed to connect or send to ",
                         peer, " with address ", self.members[peer], " message", data)
...
class Server(Thread):
    ...
    def receive_message(self, key, mask):
        ...
        if close:
            try:
                ...
                log_warn(
                    "Server: [Receive-message]warning failed to unregister event ", data.addr)
            sock.close()
    ...
    def send_message(self, key, mask):
        ...
        if data.outb:
            try:
                # Fix if connection closed or dropped
                ...
                try:
                    ...
                    log_warn(
                        "Server: [Send-message] warning failed to unregister event ", data.addr)
    ...
    def run(self):
        ...
        while True:
            if self.elect_manager.bully and not self.elect_manager.is_leader():
                is_alive = self.loop.run_until_complete(
                    self.elect_manager.send_probe_async(self.elect_manager.bully))
                if not is_alive:
                ...
                log_probe(
                    "Probe: Waiting because I don't know who is leader or I am the leader")
...
class Lab2(Thread):
    ...
    def run(self):
        log_client(
            "############ Client Initiate ################################")
        log_client('Client: STARTING WORK for pid {} on {}'.format(
            self.process_id, self.address))
        log_client("############################################################")
        ...
        while True:
            ...
            if self.elect_manager.elect_mode:
                is_leader = self.loop.run_until_complete(
                    self.elect_manager.elect_and_announce())
                self.elect_manager.unset_elect_mode()
            else:
    ...
    def join_group(self):
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as gcd:
                ...
                self.elect_manager.update_members(
                    message(gcd, ('JOIN', (self.process_id, self.address))))
        except:
